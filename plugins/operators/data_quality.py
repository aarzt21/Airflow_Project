...
class DataQualityOperator(BaseOperator):
    ...
    def execute(self, context):
        ...
        for table in self.tables:
            ...
            if num_records == 0:
                self.log.error("No records exist in {}".format(table))
                raise ValueError("No records exist in {}".format(table))
            self.log.info("Table {} passed sanity check with {} records".format(table, num_records))